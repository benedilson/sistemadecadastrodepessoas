...
import br.com.scp.model.Pessoa;
...
public class PessoaEndpoint {
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<Pessoa> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT p FROM Pessoa p LEFT JOIN FETCH p.loginOperador WHERE p.id = :entityId ORDER BY p.id",
						Pessoa.class);
	...
	public List<Pessoa> listAll(@QueryParam("start") Integer startPosition,
		...
		TypedQuery<Pessoa> findAllQuery = em
				.createQuery(
						"SELECT DISTINCT p FROM Pessoa p LEFT JOIN FETCH p.loginOperador ORDER BY p.id",
						Pessoa.class);
